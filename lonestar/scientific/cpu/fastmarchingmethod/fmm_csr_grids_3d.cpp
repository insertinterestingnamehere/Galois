...
void initBoundary(Graph& graph, BL& boundary) {
  galois::do_all(
      ...
      [&](const T& node) noexcept {
        ...
        curData.tag      = KNOWN;
        curData.solution = BoundaryCondition(id2xyz(node));
      },
...
void SetupGrids(Graph& graph) {
  if (!input_segy.empty()) {
    ...
    [[maybe_unused]] auto [num_nodes, num_cells, num_outer_faces, xy_low,
                           xy_high, yz_low, yz_high, xz_low, xz_high] =
        ConstructCsrGrids(graph, std::array<std::size_t, 3>{x, y, 1});
    ...
    for (auto node : graph) {
      ...
      [[maybe_unused]] auto [i, j, k] = id2ijk(node);
      if (i != read_traces) {
    ...
    [[maybe_unused]] auto [num_nodes, num_cells, num_outer_faces, xy_low,
                           xy_high, yz_low, yz_high, xz_low, xz_high] =
        ConstructCsrGrids(graph, std::array<std::size_t, 3>{nx, ny, nz});
    ...
    galois::do_all(
        ...
        [&](auto node) noexcept {
          ...
          curData.is_ghost = (node >= num_cells);
          curData.tag      = FAR;
          curData.speed    = SpeedFunction(id2xyz(node));
          curData.solution = INF; // TODO ghost init?
        },

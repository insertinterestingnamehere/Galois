 ...
 * SEGY reader
 *
 * Segy files are stored in Big Endian format, independent of which platform
 * they were created on. a) Determine platform. b) If Little Endian, swap endian
 * format on input.
 *
...
public:
   ...
   */
  csSegyReader(std::string filename, SegyReaderConfig const& config,
               csSegyHdrMap const* hdrMap = NULL);
  ~csSegyReader();
  /**
   * This method initialises the reader object, by reading in the SEGY char and
   * binary headers and setting all internal buffers etc.
   *
   * Call this method before starting to read in data.
   */
  void initialize();
  /// Call this method if the number of samples in the SEGY file shall be
  /// overridden
  void initialize(int numSamples);

  /**
   * Set specific trace selection.
   * Only traces matching the selection will be read in and sorted, if
   * requested.
   *
   */
  bool setSelection(std::string const& hdrValueSelectionText,
                    std::string const& headerName, int sortOrder,
                    int sortMethod);
  void setSelectionStep1(std::string const& hdrValueSelectionText,
                         std::string const& headerName, int sortOrder,
                         int sortMethod);
   ...
   *
   * @param numTracesToRead (i) Expected(!) number of traces that will be read
   * at once by repeated calls to getNextTrace(). This parameter makes sure that
   * no more than numTracesToRead will be read into a temporary buffer. This
   *                            will speed up the read process somewhat,
   * depending on the number of traces to pre-buffer. If unsure, use method call
   ...
   * Trace header values must be retrieved by calls to methods hdrIntValue()
   * etc...
   *
   * @param buffer   (o) Output buffer (sample values only, no trace header).
   * Buffer must be fully allocated to hold one full trace.
   * @param nSamples (i) Number of samples to read
   * @return true if operation was successful, false if no more trace was found
   * (end of file reached)
   */
  ...
  bool setHeaderToPeek(std::string const& headerName,
                       cseis_geolib::type_t& headerType);
  bool peekHeaderValue(cseis_geolib::csFlexHeader* hdrValue,
                       int traceIndex = -1);
  ...
  bool addHeader(int byteLoc, int byteSize, type_t inType,
                 cseis_geolib::csHeaderInfo const& hdr);
  bool addHeader(cseis_geolib::csSegyHeaderInfo const& hdr);
  bool addHeader(std::string const& theName, int theByteLoc, int theByteSize,
                 type_t theType);
  bool addHeader(std::string const& theName, int theByteLoc, int theByteSize,
                 type_t theType, std::string const& theDesc);
...
private:
  ...
  // Size in byte of one sample. Depends on data sample format, e.g. 4-byte
  // floating point.
  ...
  char* myBigBuffer;
  /// Capacity in number of traces of big buffer = Maximum number of traces that
  ...
  /// Trace index where file pointer currently stands. Reading from SEGY file
  /// the next time will read the trace with the given trace index.
  int myCurrentTraceInFile;
  /// Trace index of trace that is going to be retrieved next (either from
  /// buffer or input file)
  int myCurrentTraceRead;

  /// 'Copy' buffer. Holds data samples of one trace. Is only allocated on
  /// request (typically call from JNI interface)
  byte_t* myCopyBuffer;
  ...
  /// true if reader has been initialized, i.e. char & bin headers have been
  /// read in, and trace header is sealed
  bool myHasBeenInitialized;
  /// true if SEGY char header is formatted in EBCDIC code
  bool myIsEBCDIC;
  /// true if some particular trace headers shall be scaled automatically
  /// (geometry headers)
  bool myIsAutoScaleHeaders;
  /// true if sample format code in SEGY binary header shall be overriden by
  /// manual format code
  ...
  /// Number of samples
  int myNumSamples;
  /// Preset definition for SEGY trace header mapping, e.g.
  /// cseis_segy::HDR_MAPPING_STANDARD
  ...
  int myNumTraces;
  /// Residual number of bytes at end of SEGY file (typically 0). This may be
  /// the case for corrupted SEGY files, or when SEGY file is being written at
  /// the same time.

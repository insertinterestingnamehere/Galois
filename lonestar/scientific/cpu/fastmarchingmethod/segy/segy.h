...
float segy::toIeee(uint32_t ibm) {
  ...
  /* adjust exponent from base 16 offset 64 radix point before first digit to
   * base 2 offset 127 radix point after first digit */
  ...
  uint32_t ieee =
      ((uint32_t)(fr >> 9)) | ((uint32_t)(exp << 23)) | ((uint32_t)(sgn << 31));
  float result;

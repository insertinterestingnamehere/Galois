...
#define ADD_VALUE_AT_OFFSET_GETTER_METHOD                                      \
  ...
  T getValueAtOffset(int offset) const {                                       \
    ...
    return value;                                                              \
                                                                               ...
                                                                               \
  template <typename T>                                                        \
  typename T::type get_field(std::string = "") const {                         \
    const char* addr = data;                                                   \
    const typename T::type* val_addr =                                         \
        reinterpret_cast<const typename T::type*>(addr + (T::offset)-1);       \
    typename T::type value = *val_addr;                                        \
    value                  = fix_endianness_if_needed(value);                  \
    return value;                                                              \
  }

  ...
  galois::runtime::profileVtune(
      [&]() {
        do {
...
#endif
                  double sln_temp = solveQuadratic(
                      graph, node, node_data.solution, node_data.speed);
                  double old_sln = galois::min(node_data.solution, sln_temp);
                  if (sln_temp < old_sln) {
                    ...
                    galois::gDebug(node, " solution not updated: ", sln_temp,
          ...
          syncSubstrate
              ->sync<writeAny, readAny, Reduce_min_solution, Bitset_solution>(
                  "FastMarching");

...
void fromCsv(Graph& graph) {
  ...
  while (std::getline(incsv, line)) {
    ...
    } while (gid < NUM_CELLS && *end == ' ');
    if (gid >= NUM_CELLS)
      break;
    if (*end != '\0')
      GALOIS_DIE("Bad EOL.");
  }
  galois::gDebug(gid, "/", NUM_CELLS, " cells initialized from csv");
  // Initialize remaining nodes
  galois::do_all(
      galois::iterate(gid, NUM_CELLS),
      [&](auto gid) noexcept {
        if (!graph.isLocal(gid)) {
          return;
        }
        ...
        auto& curData    = graph.getData(node, galois::MethodFlag::UNPROTECTED);
        curData.solution = INF;
        curData.speed    = SpeedFunction(id2xy(gid));
      },
  ...
  galois::do_all(
      ...
      [&](auto node) noexcept {
        ...
        if (graph.getGID(node) >= NUM_CELLS)
          return;
        curData.speed = SpeedFunction(id2xy(graph.getGID(node))) * speed_factor;
      },

...
private:
  ...
  void readElements(std::string filename, std::vector<Tuple>& tuples) {
    if (readElementsBin<KEEP_ID>(filename, tuples))
      return;
    else
  ...
  void readPoly(std::string filename, std::vector<Tuple>& tuples) {
    if (readPolyBin<KEEP_ID>(filename, tuples))
      return;
    else
      writePoly(filename);
...
public:
  ...
  size_t read(GraphTy& graph, Args&&... args) {
    typename GraphTy::read_tag tag;
    return readDispatch(graph, tag, std::forward<Args>(args)...);
  }
  ...
  size_t readDispatch(Graph& mesh, galois::graphs::read_with_aux_first_graph_tag tag,
    ...
    readPoly<true>(basename, tuples);
    makeGraph(mesh, tag, parallelAllocate);
    return tuples.size();
  }
